        ...
        int row, int column)
{
   int32_t sum = 0; 
        ...
        //iterate over the entire filter
        for(int32_t filter_row = -radius; filter_row<=radius; filter_row++){
            for(int32_t filter_col = -radius; filter_col <= radius; filter_col++){ //Smart way to go around the entire matrix
                int32_t image_row = row + filter_row; 
                int32_t image_col = col + filter_col; 
                
                if(image_row < 0 || image_row >= height || image_col < 0 || image_col >= width) continue; // Covering literal edge cases

                int32_t pixel = original[image_row * width + image_col]; //width being the offset
                int32_t filter_value = f->matrix[(filter_row + radius) * n + (filter_col + radius)]; // n being the offset for the filter
        ...
        int32_t width, int32_t height)
{
  ...
  int32_t radius = n/2; //To see where we need to look
  ...
  for(int32_t row = 0; row < height; row++){
    for(int32_t col =0; col < width; col++){
        ...
        int32_t sum  = apply2d(f, original, target, row,col);
        ...
        int32_t num_threads, parallel_method method, int32_t work_chunk)
{
     ...
     */
     ...
     if(num_threads <= 1){ //single threaded case 
        apply_filter2d(f,original,target, width, height);
        return;
     }
